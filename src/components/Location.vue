...
export default defineComponent({
  ...
  methods: {
    onUpdatedCity(text) {
      ...
      axios.get("https://bot-test.fun/location/?cityName=" + text).then(res => { // Запрос данных с бека
        this.cityOptions = res.data; // Ответ на фронт
      })
    },
    onUpdatedStreet(text) {
      this.formData.street = text;
      axios.get("https://bot-test.fun/location/?cityName=" + this.formData.city + "&streetName=" + text).then(res => { // Запрос данных с бека
