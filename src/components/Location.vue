...
export default defineComponent({
  ...
  methods: {
    onUpdatedCity(text) {
      ...
      axios.get("https://f7b3-93-170-55-154.eu.ngrok.io/location/?cityName=" + text).then(res => { // Запрос данных с бека
        this.cityOptions = res.data; // Ответ на фронт
      })
    },
    onUpdatedStreet(text) {
      this.formData.street = text;
      axios.get("https://f7b3-93-170-55-154.eu.ngrok.io/location/?cityName=" + this.formData.city + "&streetName=" + text).then(res => { // Запрос данных с бека
        this.streetOptions = res.data; // Ответ на фронт
      })
    },
    onGetCoordinates(lat, lng) {
      axios.get("https://f7b3-93-170-55-154.eu.ngrok.io/location/auto/?lat=" + lat + "&lng=" + lng).then(res => {
